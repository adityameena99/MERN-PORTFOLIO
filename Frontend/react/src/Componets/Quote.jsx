...
export default function Quote() {
  ...
  useLayoutEffect(() => {
    ...
    letters.forEach(l => gsap.set(l, { display: "inline-block", x: 0, y: 0, rotation: 0, opacity: 1 }));
        ...
        {textLines.map((line, i) => (
          <React.Fragment key={i}>
            {line.split("").map((char, idx) => {
              const color = (line === "DECORATION" && char !== " ") ? "#A9A9A9" : "white";
              return <span key={i + "-" + idx} style={{ color }}>{char}</span>;
            })}
            <br />
          </React.Fragment>
        ))}
