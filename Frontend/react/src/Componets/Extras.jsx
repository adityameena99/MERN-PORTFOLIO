...
export default function Extras() {
  ...
  useEffect(() => {
    ...
    const genVeins = () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        veins.push({ sx, sy, cx1, cy1, ex, ey, a: 0.035 + Math.random() * 0.07, lw: 0.45 + Math.random() * 1.0 });
      }
    };
    genVeins();
    ...
    const draw = () => {
      ...
      const baseG = ctx.createRadialGradient(sCx - r * 0.1, sCy - r * 0.1, r * 0.02, sCx, sCy, r);
      ...
      for (const v of veins) {
        ...
        ctx.quadraticCurveTo(v.cx1 + sclX, v.cy1 + sclY, v.ex + sclX, v.ey + sclY);
      ...
      const invX = -(mouseX - W / 2) * 0.08;
      const invY = -(mouseY - H / 2) * 0.08;
      const lightX = sCx - r * 0.32 + invX;
      const lightY = sCy - r * 0.36 + invY;
      const lightGrad = ctx.createRadialGradient(lightX, lightY, r * 0.035, sCx, sCy, r * 0.92);
      ...
      ctx.beginPath();
      ctx.ellipse(-r * 0.32 + invX * 0.55, -r * 0.42 + invY * 0.55, r * 0.26, r * 0.16, -0.36, 0, Math.PI * 2);
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden", background: "#FFFFFF", position: "relative" }}>
      {/* Canvas sclera */}
      <canvas ref={canvasRef} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", zIndex: 11111,   pointerEvents: "none" }} />
      ...
      <div
        ...
        style={{
          ...
          border: "none",
          transition: "transform 0.06s linear, width 0.18s linear, height 0.18s linear",
        }}
      >
        <img
...
>
  <FaPlay className="play-icon" style={{ color: "green", width: "28px", height: "28px", zIndex: 4 }} />
</div>
